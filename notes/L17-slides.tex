...
\begin{changemargin}{1cm}
  \Large
(But we've seen better: \\ \qquad coverage-guided fuzzing.)
\end{changemargin}
...
\begin{frame}[fragile]
  ...
  \begin{changemargin}{2cm}
    ...
    Bolero: abstracts away from specific fuzzing engine.
  \end{changemargin}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Bolero at work}
\begin{lstlisting}[language=Rust,basicstyle=\scriptsize]
#[test]
#[cfg_attr(kani, kani::proof)]
fn test_update_account_balance_bolero() {
    bolero::check!().with_type::<(i32, i32)>().cloned().
                  for_each(|(current_balance, amount)| {
        let result = update_account_balance(current_balance, amount);
        assert!(balance_update_is_correct(current_balance, 
                  amount, result));
    });
}
\end{lstlisting}
...
\begin{frame}[fragile]
  ...
  \begin{changemargin}{1cm}
    ...
    \item Kani and libfuzzer succeeded and failed on same programs.
      \item but this is not true in general.
    \end{itemize}
~\\
...
\begin{frame}[fragile]
  ...
  \begin{changemargin}{1cm}
    \Large
    Both fuzzing and Kani might give \\ \qquad a concrete test case; \\
    it's real, and you can fix it.\\[1em]
  \end{changemargin}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Interpreting Kani result}
  \begin{changemargin}{1cm}
    \Large
    Kani success means: \\ \qquad your program is correct modulo the bound,\\
    assuming everything else on the system correct.\\[1em]
  \end{changemargin}
\end{frame}

\usebackgroundtemplate{\tikz\node[opacity=0.1]{\includegraphics[width=\paperwidth]{L17/05409_mossy_rock_v1.JPG}};}
\begin{frame}[fragile]
  \frametitle{Interpreting fuzzing result}
  \begin{changemargin}{1cm}
    \Large
    Fuzzing says nothing: \\ \qquad don't really know that your program is correct,\\
    \qquad but have some warm and fuzzy feelings.
  \end{changemargin}
\end{frame}
\usebackgroundtemplate{}
...
\begin{frame}[fragile]
  ...
  \begin{changemargin}{1cm}
    \Large
    Bolero approach: \\
    \qquad \texttt{kani::any()} to get a symbolic value, \\
    \qquad \texttt{kani::assume()} to restrict the range, e.g.
    \normalsize
\begin{lstlisting}
...
\large
\begin{lstlisting}[language=dafny,basicstyle=\small]
  class Rectangle {
    var width:int
    var height:int
  }
...
\large
  \begin{changemargin}{1cm}
    ...
    More generally: this concrete test case is concrete; \\
    we can also write \texttt{assert}, in which case
    we need the method to have an \texttt{ensures} clause.\\[1em]
    If we have \texttt{ensures} then we can statically verify the whole method under all inputs, with \texttt{assert}.
  \end{changemargin}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{changemargin}{1cm}
    ...
    But, here, also reports a failure!
{ \scriptsize
\begin{verbatim}
...
\usebackgroundtemplate{\tikz\node[opacity=0.3]{\includegraphics[width=\paperwidth]{L17/PXL_20220729_181423529.jpg}};}
...
\begin{frame}
  \frametitle{Cedar example: TinyTodo}
  \begin{center}
    \includegraphics[width=.6\textwidth]{L17/todo.png}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{TinyTodo and Cedar}
    ...
    \begin{changemargin}{1cm}
      ...
      TinyTodo doesn't contain auth reasoning code.\\[1em]
      It uses Cedar to say whether an action is allowed or denied.
    \end{changemargin}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Verification-guided development}
    ...
    \begin{changemargin}{2cm}
      ...
      \begin{enumerate}
      \item explicit properties \\ \quad (specified by developers); and
      \item implicit properties \\ \quad (must always be true).
      \end{enumerate}
...
\begin{frame}[fragile]
  \frametitle{Deployment and models}
    ...
    \begin{changemargin}{2cm}
      ...
      In the Cedar example: Dafny is a model of a parallel Rust codebase.\\[1em]
      Verifying the Dafny is still useful; it implies that a correct implementation is possible.
    \end{changemargin}
\end{frame}

\usebackgroundtemplate{\tikz\node[opacity=0.2]{\includegraphics[width=\paperwidth]{L05/960px-Delphi_Temple_of_Apollo.jpeg}};}
\begin{frame}[fragile]
...
\begin{frame}[fragile]
  \frametitle{Assertions in Rust}
    \Large
    \begin{changemargin}{2cm}
      We can write the properties from the model.\\[1em]
      But what to do if they fail?
    \end{changemargin}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Idea: \alert{differential random testing}}
    ...
    \begin{changemargin}{2cm}
      ...
      Like fuzzing, generate millions of inputs; \\ \qquad then check equivalence.
    \end{changemargin}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Example Request}
    ...
    \begin{changemargin}{2cm}
      ...
      If this is true, the request then \emph{satisfies} the policy. \\[1em]
...
\end{lstlisting}
\begin{changemargin}{2cm}
  Why is this wrong with respect to key properties?
\end{changemargin}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Patching the bug with a predicate}
    \Large
\begin{lstlisting}[language=dafny]
...
\begin{frame}[fragile]
  ...
  \large
  \begin{changemargin}{1cm}
    Predicate \texttt{isExplicitlyPermitted}: must be a specific policy \texttt{p} permitting \texttt{request} (key property 1).\\[1em]
...
\begin{frame}[fragile]
  ...
  \Large
  \begin{changemargin}{2cm}
    Notably, that the Cedar validator is sound:\\[1em]
...
\begin{frame}[fragile]
  ...
  \begin{changemargin}{2cm}
    ...
    \begin{itemize}
    \item automated reasoning, by Dafny;
      \end{itemize}
      but also
    \begin{itemize}
    ...
    Even good Rust is still an implementation language with lots of implementation details.\\[1em]
...
\begin{frame}[fragile]
  ...
  \large
  \begin{changemargin}{1cm}
      Inputs aren't completely randomly generated. \\[1em]
      Same techniques as talked about in this class:\\ input generators to create consistent inputs.\\[1em]
      6 hours nightly of DRT, 100 million total tests.\\[1em]
      Found errors in libraries \\ \qquad (mostly parsers and IP address operations).
  \end{changemargin}
\end{frame}
