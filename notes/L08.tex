...
\lecture{8 --- January 30, 2026}{Winter 2026}{Patrick Lam}{version 1}
...
Today's code is in the repo in the \texttt{L08/mutation\_fuzzer.py} file.
...
that the \texttt{MutationFuzzer} generates. If $i$ leads to new coverage (not in \texttt{.coverages\_seen}), MCF adds $i$ to the \texttt{.population}.
...
\paragraph{Summary.} In any case, coverage-guided fuzzing definitely explores new parts of the program's behaviour as it runs. Of course, as with any type of fuzzing,
...
\begin{lstlisting}[language=Python]
  >>> mutation__coverage_fuzzer = MutationCoverageFuzzer(seed=[seed_input])
>>> mutation_coverage_fuzzer.runs(http_runner, trials=10000)
>>> mutation_coverage_fuzzer.population[:5]
['http://www.google.com/search?q=fuzzing',
