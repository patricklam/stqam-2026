...
Let's see what happens if we run this 1000 times with random inputs, using the \texttt{fuzzer()} from last time to generate these inputs, rather than mutating existing inputs (in \texttt{code/L08/random_inputs.py}):
\begin{lstlisting}[language=Python]
  for i in range(1000):
    try:
        fuzzer = Fuzzer()
