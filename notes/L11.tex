...
Let's do the same thing for reduction: we parse the input
and then we manipulate the derivation tree, to reduce it.
...
we can swap the \brac{expr} we see in the above figure
...
we can use it to do a like-for-like replacement.
...
\texttt{2} and then \texttt{3}. But, delta debugging tries to remove
more before removing less---it prefers to remove larger chunks
before removing smaller chunks. In the tree context, we want to replace
with larger subtrees instead of smaller subtrees.
...
that goes to the \brac{digit} 3. There is nothing more,
