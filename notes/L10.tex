...
>>>     print(f.fuzz())
 ...
 foo.py
\end{python}
Using a \texttt{GrammarCoverageFuzzer} would be much better, but we didn't explain that, and
there is also a lot of implementation for that class.

You can actually run \texttt{autopep8} with these inputs as well, but we won't show that.
