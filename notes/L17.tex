...
There exist both user-provided properties (you write those), as well as implicit properties.
Dafny
formally verifies that the code respects the properties.  It's better
than testing, in that the property is guaranteed to be true of the
code.

Maybe we don't actually want to deploy Dafny code in
production. (Actually, it is possible to compile Dafny to
other languages, eg C\#, Java, JavaScript, Go, and Python, and deploy that). In this application, the Dafny code we're talking about is best thought of as a
