...
at most the number of images with a tag.

In other words: let's say that there are two tags, ``a'' and ``b'', and
four images.
Images \{1, 2, 3\} are tagged ``a'', while images \{1, 2, 4\} are tagged
``b''. If you request all images tagged either ``a'' or ``b'',
then you should get 3 images---but certainly at most 4.
...
the tags. Next, the test must then call \texttt{pwg.tags.getImages},
passing it all of the reported tag ids, and creating an overall set
of images which have any of the tags. Finally, the test must assert
that the sum of the number of images returned by \texttt{getList} is at most
the number of images returned by \texttt{getImages}.
